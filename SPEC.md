...
repo on disk
   |
   v
indexer (ast or tree-sitter, git log)  --> SQLite (nodes, edges) --> NetworkX (in-memory, for traversals)
                                                   |
                                                   v
                              agent (LangChain or plain tool-calling loop)
                                 tools: graph tools + grep + read_file
                                                   |
                                                   v
...
- **Storage:** SQLite tables `nodes`, `edges`, plus FTS5 over names and docstrings. Load into NetworkX for multi-hop traversal.
- **Incremental indexing:** hash each file; re-parse only changed files and their dependents.
- **Agent:** tool-calling loop. The model chooses between graph tools and grep. Tool results always include file and line.
...
- **Baselines:** (a) grep/read-only agent, (b) vector-RAG over code chunks, (c) graph-tool agent (ours).
...
- **M0, Indexer:** parse Python repo to SQLite (files, classes, functions, imports, CONTAINS/IMPORTS/INHERITS). CLI: `index <path>`.
...
- **M3, Agent:** tool-calling agent with the tools in section 7, CLI `ask "<question>"`, evidence-cited answers.
...
- SQLite + NetworkX. No graph database in v1.
- LangChain or a plain tool-calling loop (decide in M3; prefer the simpler option).
